	...
	while (!coda.empty())
	{
		...
		for (i = cabine[c].corridoi.begin() ; i != cabine[c].corridoi.end(); i++)
		{
			if (i->lunghezza < cabine[c].ossigeno_rimasto)
			{
				...
				//Visita la cabina anche se non migliorando la distanza migliora la quantità di ossigeno rimasto,
				//tranne che se la cabina è quella del tesoro
				if (cabine[c].ossigeno_rimasto - i->lunghezza > cabine[i->porta_a].ossigeno_rimasto && i->porta_a != C)
				{
					cabine[i->porta_a].distanza = cabine[c].distanza + i->lunghezza;
					cabine[i->porta_a].ossigeno_rimasto = cabine[c].ossigeno_rimasto - i->lunghezza;
					coda.push(i->porta_a);
				}
			}
		}
